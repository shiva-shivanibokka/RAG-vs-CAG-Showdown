...
async def lifespan(app: FastAPI):
    global _cag, _rag
    logger.info("Loading engines on startup (model=%s)...", OPENAI_MODEL)
    _cag = CAGEngine(KNOWLEDGE_BASE, model=OPENAI_MODEL)
    _rag = RAGEngine(KNOWLEDGE_BASE, model=OPENAI_MODEL, top_k=RAG_TOP_K)
...
@app.get("/health")
def health():
    return {"status": "ok", "model": OPENAI_MODEL, "provider": "OpenAI"}
