...
from src.config import API_ROOT_PATH, CORS_ORIGINS, CEREBRAS_MODEL, RAG_TOP_K
...
async def lifespan(app: FastAPI):
    global _cag, _rag
    logger.info("Loading engines on startup (model=%s)...", CEREBRAS_MODEL)
    _cag = CAGEngine(KNOWLEDGE_BASE, model=CEREBRAS_MODEL)
    _rag = RAGEngine(KNOWLEDGE_BASE, model=CEREBRAS_MODEL, top_k=RAG_TOP_K)
...
@app.get("/health")
def health():
    return {"status": "ok", "model": CEREBRAS_MODEL, "provider": "Cerebras"}
